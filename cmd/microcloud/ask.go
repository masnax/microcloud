...
func (c *initConfig) askMissingServices(services []types.ServiceType, stateDirs map[types.ServiceType]string) ([]types.ServiceType, error) {
	...
	if len(missingServices) > 0 {
		...
		if !c.autoSetup {
			confirm, err := c.asker.AskBool(fmt.Sprintf("%s not found. Continue anyway?", serviceStr), true)
			if err != nil {
				return nil, err
			}
	...
	if listenAddr == "" {
		...
		if !c.autoSetup && len(info) > 1 {
			...
			err := c.askRetry("Retry selecting an address?", func() error {
				answers, err := table.Render(context.Background(), "Select an address for MicroCloud's internal traffic:")
				if err != nil {
					return err
				}
...
func (c *initConfig) askLocalPool(sh *service.Handler) error {
	...
	err = c.askRetry("Retry selecting disks?", func() error {
		...
		answers, err := table.Render(context.Background(), "Select exactly one disk from each cluster member:")
		if err != nil {
			return err
		}
		...
		if !c.wipeAllDisks && wipeable {
			...
			answers, err := table.Render(context.Background(), "Select which disks to wipe:")
			if err != nil {
...
func (c *initConfig) askRemotePool(sh *service.Handler) error {
	...
	if len(askSystemsRemote) != 0 {
		...
		if len(askSystemsRemote) != availableDiskCount && wantsDisks {
			wantsDisks, err = c.asker.AskBool("Unable to find disks on some systems. Continue anyway?", true)
			if err != nil {
				return err
			}
		}
		...
		err = c.askRetry("Change disk selection?", func() error {
			...
			table := tui.NewSelectableTable(header, data)
			selected, err := table.Render(context.Background(), "Select from the available unpartitioned disks:")
			if err != nil {
				return err
			}
			...
			if len(selected) > 0 && !c.wipeAllDisks {
				...
				toWipe, err = table.Render(context.Background(), "Select which disks to wipe:")
				if err != nil {
					return err
				}
			}
...
func (c *initConfig) askOVNNetwork(sh *service.Handler) error {
	if sh.Services[types.MicroOVN] == nil {
		fmt.Println("MicroOVN not installed")
		return nil
	}
	...
	for _, state := range c.state {
		...
		if !supportsOVN || len(state.AvailableUplinkInterfaces) == 0 {
			fmt.Println("Skipping OVN network setup, some systems don't support it")
			return nil
		}
	...
	if len(askSystems) == 0 {
		fmt.Println("ask systems is empty")
		return nil
	}
	...
	for name, state := range c.state {
		...
		if len(state.AvailableUplinkInterfaces) == 0 {
			wantsContinue, err := c.asker.AskBool("Some systems are ineligible for distributed networking, which requires either an interface with no IPs assigned or a bridge. Continue anyway?", true)
			if err != nil {
				return err
			}
	...
	err = c.askRetry("Retry selecting uplink interfaces?", func() error {
		answers, err := table.Render(context.Background(), "Select an available interface per system to provide external connectivity for distributed network(s):")
		if err != nil {
			return err
		}
	...
	for peer, system := range c.systems {
		if len(c.state[system.ServerInfo.Name].AvailableOVNInterfaces) == 0 {
			fmt.Printf(tui.WarningColor(fmt.Sprintf("Not enough interfaces available on %s to create an underlay network, skipping\n", peer), false))
	...
	if len(ovnUnderlayData) != 0 && canOVNUnderlay {
		wantsDedicatedUnderlay, err := c.asker.AskBool("Configure dedicated underlay networking?", false)
		if err != nil {
			return err
		}
		...
		if wantsDedicatedUnderlay {
			...
			err = c.askRetry("Retry selecting underlay network interfaces?", func() error {
				answers, err := table.Render(context.Background(), "Select exactly one network interface from each cluster member:")
				if err != nil {
					return err
				}
	...
	for peer, system := range c.systems {
		...
		if ovnUnderlaySelectedIPs != nil {
			...
			if ok {
				...
				fmt.Println(tui.SummarizeResult("Using %s for OVN underlay traffic on %s", ip.String(), peer))
...
func (c *initConfig) askNetwork(sh *service.Handler) error {
	...
	for _, state := range c.state {
		...
		if !supportsFAN {
			proceedWithNoOverlayNetworking, err := c.asker.AskBool("FAN networking is not usable. Do you want to proceed with setting up an inoperable cluster?", false)
			if err != nil {
				return err
			}
...
func (c *initConfig) askCephNetwork(sh *service.Handler) error {
	...
	for name, state := range c.state {
		if len(state.AvailableCephInterfaces) == 0 {
			fmt.Printf(tui.WarningColor(fmt.Sprintf("No network interfaces found with IPs on %q to set a dedicated Ceph network, skipping Ceph network setup\n", name), false))
...
func (c *initConfig) askJoinIntents(gw *cloudClient.WebsocketGateway, expectedSystems []string) ([]types.SessionJoinPost, error) {
	...
	if !c.autoSetup {
		...
		err := c.askRetry("Retry selecting systems?", func() error {
			var err error
			answers, err = table.Render(gw.Context(), "Select which systems you want to join:")
			if err != nil {
...
func (c *initConfig) askJoinConfirmation(gw *cloudClient.WebsocketGateway) error {
	...
	if !c.autoSetup {
		...
		fmt.Println("")
		fmt.Println(tui.ErrorColor("Do not exit out to keep the session alive!", true))
	...
	fmt.Println(tui.SuccessColor("Successfully joined the cluster", true))
	return nil
}
