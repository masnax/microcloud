...
func (c *initConfig) askUpdateProfile(profile api.ProfilesPost, profiles []string, lxdClient lxd.InstanceServer) (*api.ProfilePut, error) {
	...
	if !c.autoSetup && len(askConflictingConfig) > 0 || len(askConflictingDevices) > 0 {
		replace, err := c.asker.AskBool("Replace existing default profile configuration?", "no")
		if err != nil {
			return nil, err
		}
...
func (c *initConfig) askRetry(question string, f func() error) error {
	for {
		...
		err := f()
		if err != nil {
			fmt.Println(style.ErrorColor(err.Error()))

			if !c.autoSetup {
				retry, err = c.asker.AskBool(fmt.Sprintf("%s", question), "yes")
				if err != nil {
					return err
				}
			}
...
func (c *initConfig) askMissingServices(services []types.ServiceType, stateDirs map[types.ServiceType]string) ([]types.ServiceType, error) {
	...
	if len(missingServices) > 0 {
		...
		if !c.autoSetup {
			confirm, err := c.asker.AskBool(fmt.Sprintf("%s not found. Continue anyway?", serviceStr), "yes")
			if err != nil {
				return nil, err
			}
...
func (c *initConfig) askAddress(filterAddress string) error {
	...
	if listenAddr == "" {
		...
		if !c.autoSetup && len(info) > 1 {
			...
			table := style.NewSelectableTable([]string{"ADDRESS", "IFACE"}, data)
			err := c.askRetry("Retry selecting an address?", func() error {
				fmt.Println(style.SetColor(style.White, "Select an address for MicroCloud's internal traffic:", true))
				answers, err := table.Render(context.Background())
				if err != nil {
					return err
				}
				...
				fmt.Printf("\n%s\n\n", style.SummarizeResult("Using address %s for MicroCloud", listenAddr))
			...
			fmt.Println(style.SummarizeResult("Using address %s for MicroCloud", listenAddr))
		}
	}
...
func (c *initConfig) askLocalPool(sh *service.Handler) error {
	...
	wantsDisks := true
	if !c.autoSetup {
		wantsDisks, err = c.asker.AskBool("Would you like to set up local storage?", "yes")
		if err != nil {
			return err
		}
	}
	...
	if !c.autoSetup {
		err := c.askRetry("Retry selecting disks?", func() error {
			...
			table := style.NewSelectableTable(header, data)
			fmt.Println(style.SetColor(style.White, "Select exactly one disk from each cluster member:", true))
			answers, err := table.Render(context.Background())
			if err != nil {
				return err
			}
			...
			if !c.wipeAllDisks && wipeable {
				fmt.Println(style.SetColor(style.White, "Select which disks to wipe:", true))
				err := table.ReplaceRows(answers)
				if err != nil {
					return err
				}
	...
	for target, path := range selectedDisks {
		fmt.Println(style.SummarizeResult("Using %s on %s for local storage pool", path, target))
	}
...
func validateCephInterfacesForSubnet(lxdService *service.LXDService, systems map[string]InitSystem, availableCephNetworkInterfaces map[string]map[string]service.DedicatedInterface, askedCephSubnet string) error {
	...
	for _, interfaces := range validatedCephInterfacesData {
		for _, iface := range interfaces {
			...
			fmt.Println("Interface %s (%s) detected on cluster member %s", iface[1], iface[2], iface[0])
		}
	}
...
func (c *initConfig) askRemotePool(sh *service.Handler) error {
	...
	if len(askSystemsRemote) != 0 {
		...
		if availableDiskCount == 0 {
			fmt.Println(style.WarningColor("Warning: No disks available for distributed storage. Skipping configuration"))
		...
		wantsDisks := true
		if !c.autoSetup {
			wantsDisks, err = c.asker.AskBool("Would you like to set up distributed storage?", "yes")
			if err != nil {
				return err
			}
			...
			if len(askSystemsRemote) != availableDiskCount && wantsDisks {
				wantsDisks, err = c.asker.AskBool("Unable to find disks on some systems. Continue anyway?", "yes")
				if err != nil {
					return err
				}
			}
		...
		err = c.askRetry("Change disk selection?", func() error {
			...
			if !c.autoSetup {
				fmt.Println(style.SetColor(style.White, "Select from the available unpartitioned disks:", true))
				selected, err = table.Render(context.Background())
				if err != nil {
					return err
				}
				...
				if len(selected) > 0 && !c.wipeAllDisks {
					fmt.Println(style.SetColor(style.White, "Select which disks to wipe:", true))
					err := table.ReplaceRows(selected)
					if err != nil {
						return err
					}

					toWipe, err = table.Render(context.Background())
					if err != nil {
						return err
					}
				}
		...
		for target, disks := range selectedDisks {
			if len(disks) > 0 {
				fmt.Println(style.SummarizeResult("Using %d disk(s) on %s for remote storage pool", len(disks), target))
			}
		}
	...
	if !c.autoSetup && !c.encryptAllDisks && len(selectedDisks) > 0 {
		var err error
		encryptDisks, err = c.asker.AskBool("Do you want to encrypt the selected disks?", "no")
		if err != nil {
			return err
		}
	}
	...
	if !useJoinConfigRemoteFS {
		if !c.autoSetup {
			...
			if hasCephFS {
				setupCephFS, err = c.asker.AskBool("Would you like to set up CephFS remote storage?", "yes")
				if err != nil {
					return err
				}
			}
...
func (c *initConfig) askOVNNetwork(sh *service.Handler) error {
	...
	wantsOVN, err := c.asker.AskBool("Configure distributed networking?", "yes")
	if err != nil {
		return err
	}
	...
	for name, state := range c.state {
		...
		if len(state.AvailableUplinkInterfaces) == 0 {
			wantsContinue, err := c.asker.AskBool("Some systems are ineligible for distributed networking, which requires either an interface with no IPs assigned or a bridge. Continue anyway?", "yes")
			if err != nil {
				return err
			}
	...
	fmt.Println(style.SetColor(style.White, "Select an available interface per system to provide external connectivity for distributed network(s):", true))
	data := [][]string{}
	for peer, state := range c.state {
		if !askSystems[peer] {
			continue
	...
	for peer, iface := range selectedIfaces {
		fmt.Println(style.SummarizeResult("Using %s on %s for OVN uplink", iface, peer))
	}
	...
	for peer, system := range c.systems {
		if len(c.state[system.ServerInfo.Name].AvailableOVNInterfaces) == 0 {
			fmt.Printf(style.WarningColor(fmt.Sprintf("Not enough interfaces available on %s to create an underlay network, skipping\n", peer)))
	...
	if len(ovnUnderlayData) != 0 && canOVNUnderlay {
		wantsDedicatedUnderlay, err := c.asker.AskBool("Configure dedicated underlay networking?", "no")
		if err != nil {
			return err
		}
		...
		if wantsDedicatedUnderlay {
			...
			fmt.Println(style.SetColor(style.White, "Select exactly one network interface from each cluster member:", true))

			table = style.NewSelectableTable(header, ovnUnderlayData)
	...
	for peer, system := range c.systems {
		...
		if ovnUnderlaySelectedIPs != nil {
			...
			if ok {
				...
				fmt.Println(style.SummarizeResult("Using %s for OVN underlay traffic on %s", ip.String(), peer))
...
func (c *initConfig) askNetwork(sh *service.Handler) error {
	...
	for _, state := range c.state {
		...
		if !supportsFAN {
			proceedWithNoOverlayNetworking, err := c.asker.AskBool("FAN networking is not usable. Do you want to proceed with setting up an inoperable cluster?", "no")
			if err != nil {
				return err
			}
...
func (c *initConfig) askCephNetwork(sh *service.Handler) error {
	...
	for name, state := range c.state {
		if len(state.AvailableCephInterfaces) == 0 {
			fmt.Printf(style.WarningColor(fmt.Sprintf("No network interfaces found with IPs on %q to set a dedicated Ceph network, skipping Ceph network setup\n", name)))
	...
	if internalCephSubnet != microCloudInternalNetworkAddrCIDR {
		...
		bootstrapSystem.MicroCephInternalNetworkSubnet = fmt.Sprintf("[%s]", internalCephSubnet)
	...
	for _, serviceType := range expectedServices {
		for name, info := range c.state {
			...
			if info.ServiceClustered(serviceType) {
				...
				// TODO: Fix this special case question.
...
func (c *initConfig) askPassphrase(s *service.Handler) (string, error) {
	validator := func(password string) error {
		...
		if len(passwordSplit) != 4 {
			return fmt.Errorf("Passphrase has to contain exactly four elements(%v)", passwordSplit)
		}
	...
	fmt.Println(style.ColorPrintf(style.Format{Arg: "Verify the fingerprint %s is displayed on the other system.", Color: style.White}, style.Format{Arg: fingerprint, Color: style.Green, Bold: true}))
...
func (c *initConfig) askJoinIntents(gw *cloudClient.WebsocketGateway, expectedSystems []string) ([]types.SessionJoinPost, error) {
	...
	rows := [][]string{}
	table := style.NewSelectableTable(header, rows)
	...
	renderIntentsInteractive := func() {
		for {
			...
			case bytes := <-gw.Receive():
				...
				table.SendUpdate(style.InsertMsg{session.Intent.Name, session.Intent.Address, fingerprint})
	...
	if !c.autoSetup {
		...
		err := c.askRetry("Retry selecting systems?", func() error {
			var err error
			fmt.Println(style.SetColor(style.White, "Select which systems you want to join:", true))
			answers, err = table.Render(gw.Context())
			if err != nil {
...
func (c *initConfig) askJoinConfirmation(gw *cloudClient.WebsocketGateway) error {
	session := types.Session{}
	...
	if !c.autoSetup {
		...
		fmt.Println("")
		fmt.Println(style.SummarizeResult("Received confirmation from system %s", session.Intent.Name))
		fmt.Println("")
		fmt.Println(style.ErrorColor("Do not exit out to keep the session alive!"))
		fmt.Println(style.ColorPrintf(style.Format{Arg: "Complete the remaining configuration on %s ...", Color: style.White}, style.Format{Arg: session.Intent.Name, Color: style.Orange, Bold: true}))
	}
	...
	fmt.Println(style.SuccessColor("Successfully joined the cluster"))
	return nil
}
