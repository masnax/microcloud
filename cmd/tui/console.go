...
type testConsole struct {
	queue   *bufio.Reader // queueu of commands to send to "stdin".
...
// outputFile creates a new file that can be used as the output for the asker.
func outputFile(filePath string) (*os.File, error) {
...
func (c *testConsole) parseInput(handler *InputHandler) error {
	...
	c.out.WriteString(fmt.Sprintf(" ANSWER: {%s}\n", input))
	var action string
	switch input {
	case "table:down":
