...
func baseTableTemplate(header []string) *table.Table {
	...
	t = t.BorderStyle(lipgloss.NewStyle().Foreground(lipgloss.Color(DarkGrey)))
	t = t.StyleFunc(func(row, col int) lipgloss.Style {
		...
		if row == 0 {
			header[col] = lipgloss.NewStyle().Foreground(lipgloss.Color(White)).Bold(true).SetString(header[col]).String()
		}
