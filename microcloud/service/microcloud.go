...
func (s *CloudService) StartCloud(service *Handler, endpoints []rest.Endpoint) error {
	return s.client.Start(endpoints, nil, &config.Hooks{
		PostBootstrap: func(s *state.State, _ map[string]string) error { return service.StopBroadcast() },
		PostJoin:      func(s *state.State, _ map[string]string) error { return service.StopBroadcast() },
