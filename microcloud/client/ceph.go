...
func (c *CephClient) NewCluster(name string, address string, timeout time.Duration) error {
	...
	if timeout == 0 {
		return c.Query(context.TODO(), "POST", api.NewURL().Path("services", "ceph", "control"), ControlPost{Bootstrap: true, Address: addr, Name: name}, nil)
	}

	ctx, cancel := context.WithTimeout(context.TODO(), timeout)
	defer cancel()

	return c.Query(ctx, "POST", api.NewURL().Path("services", "ceph", "control"), ControlPost{Bootstrap: true, Address: addr, Name: name}, nil)
}
...
func (c *CephClient) JoinCluster(name string, address string, token string, timeout time.Duration) error {
	...
	if timeout == 0 {
		return c.Query(context.TODO(), "POST", api.NewURL().Path("services", "ceph", "control"), ControlPost{JoinToken: token, Address: addr, Name: name}, nil)
	}

	ctx, cancel := context.WithTimeout(context.TODO(), timeout)
	defer cancel()

	return c.Query(ctx, "POST", api.NewURL().Path("services", "ceph", "control"), ControlPost{JoinToken: token, Address: addr, Name: name}, nil)
}
...
func (c *CephClient) NewJoinToken(name string) (string, error) {
	...
	var token string
	err := c.Query(ctx, "POST", api.NewURL().Path("services", "ceph", "tokens"), TokenRecord{Name: name}, &token)
	if err != nil {
		return "", err
	}
...
func (c *CephClient) GetClusterMembers(ctx context.Context) ([]ClusterMember, error) {
	...
	err := c.Query(ctx, "GET", api.NewURL().Path("services", "ceph", "cluster"), nil, &members)
	if err != nil {
		return nil, err
	}
...
func (c *CephClient) AddDisk(ctx context.Context, data *cephTypes.DisksPost) error {
	...
	err := c.Query(queryCtx, "POST", api.NewURL().Path("services", "ceph", "disks"), data, nil)
	if err != nil {
...
func (c *CephClient) GetDisks(ctx context.Context) (cephTypes.Disks, error) {
	...
	err := c.Query(queryCtx, "GET", api.NewURL().Path("services", "ceph", "disks"), nil, &disks)
	if err != nil {
...
func (c *CephClient) GetResources(ctx context.Context) (*api.ResourcesStorage, error) {
	...
	err := c.Query(queryCtx, "GET", api.NewURL().Path("services", "ceph", "resources"), nil, &storage)
	if err != nil {
