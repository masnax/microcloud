...
func (c *LXDClient) GetServer() (*api.Server, string, error) {
	...
	err := c.Query(ctx, "GET", api.NewURL().Path("services", "lxd", ""), nil, &server)
	if err != nil {
		return nil, "", err
	}
...
func (c *LXDClient) UpdateServer(server api.ServerPut, ETag string) error {
	...
	return c.Query(ctx, "PUT", api.NewURL().Path("services", "lxd", ""), server, nil)
}
...
func (c *LXDClient) GetStoragePoolNames() ([]string, error) {
	...
	err := c.Query(ctx, "GET", api.NewURL().Path("services", "lxd", "storage-pools"), nil, &urls)
	if err != nil {
		return nil, err
	}
...
func (c *LXDClient) DeleteStoragePool(name string) error {
	...
	return c.Query(ctx, "DELETE", api.NewURL().Path("services", "lxd", "storage-pools", name), nil, nil)
}
...
func (c *LXDClient) GetStoragePool(name string) (*api.StoragePool, string, error) {
	...
	err := c.Query(ctx, "GET", api.NewURL().Path("services", "lxd", "storage-pools", name), nil, &pool)
	if err != nil {
		return nil, "", err
	}
...
func (c *LXDClient) UpdateStoragePool(name string, pool api.StoragePoolPut, ETag string) error {
	...
	return c.Query(ctx, "PUT", api.NewURL().Path("services", "lxd", "storage-pools", name), pool, nil)
}
...
func (c *LXDClient) CreateStoragePool(pool api.StoragePoolsPost) error {
	...
	return c.Query(ctx, "POST", api.NewURL().Path("services", "lxd", "storage-pools"), pool, nil)
}
...
func (c *LXDClient) GetProfileNames() ([]string, error) {
	...
	urls := []string{}
	err := c.Query(ctx, "GET", api.NewURL().Path("services", "lxd", "profiles"), nil, &urls)
	if err != nil {
		return nil, err
	}
...
func (c *LXDClient) GetProfile(name string) (*api.Profile, string, error) {
	...
	profile := api.Profile{}
	err := c.Query(ctx, "GET", api.NewURL().Path("services", "lxd", "profiles", name), nil, &profile)
	if err != nil {
		return nil, "", err
	}
...
func (c *LXDClient) UpdateProfile(name string, profile api.ProfilePut, ETag string) error {
	...
	return c.Query(ctx, "PUT", api.NewURL().Path("services", "lxd", "profiles", name), profile, nil)
}
...
func (c *LXDClient) CreateProfile(profile api.ProfilesPost) error {
	...
	return c.Query(ctx, "POST", api.NewURL().Path("services", "lxd", "profiles"), profile, nil)
}
...
func (c *LXDClient) DeleteProfile(name string) error {
	...
	return c.Query(ctx, "DELETE", api.NewURL().Path("services", "lxd", "profiles", name), nil, nil)
}
...
func (c *LXDClient) GetNetwork(name string) (*api.Network, string, error) {
	...
	err := c.Query(ctx, "GET", api.NewURL().Path("services", "lxd", "networks", name), nil, &network)
	if err != nil {
		return nil, "", err
	}
...
func (c *LXDClient) DeleteNetwork(name string) error {
	...
	return c.Query(ctx, "DELETE", api.NewURL().Path("services", "lxd", "networks", name), nil, nil)
}
...
func (c *LXDClient) UpdateNetwork(name string, network api.NetworkPut, ETag string) error {
	...
	return c.Query(ctx, "PUT", api.NewURL().Path("services", "lxd", "networks", name), network, nil)
}
...
func (c *LXDClient) CreateNetwork(network api.NetworksPost) error {
	...
	return c.Query(ctx, "POST", api.NewURL().Path("services", "lxd", "networks"), network, nil)
}
...
func (c *LXDClient) GetCluster() (*api.Cluster, string, error) {
	...
	cluster := api.Cluster{}
	err := c.Query(ctx, "GET", api.NewURL().Path("services", "lxd", "cluster"), nil, &cluster)
	if err != nil {
		return nil, "", err
	}
...
func (c *LXDClient) UpdateCluster(cluster api.ClusterPut, ETag string) error {
	...
	return c.Query(ctx, "PUT", api.NewURL().Path("services", "lxd", "cluster"), cluster, nil)
}
...
func (c *LXDClient) CreateClusterMember(cluster api.ClusterMembersPost) (*api.ClusterMemberJoinToken, error) {
	...
	err := c.Query(ctx, "POST", api.NewURL().Path("services", "lxd", "cluster", "members"), cluster, &token)
	if err != nil {
		return nil, err
	}
