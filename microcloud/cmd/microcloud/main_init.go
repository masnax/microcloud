...
func waitForJoin(sh *service.Handler, clusterSize int, secret string, peer string, cfg types.ServicesPut) error {
  ...
  defer cancel()
	...
	select {
	case <-ctx.Done():
		return fmt.Errorf("Timed out waiting for a response from %q", peer)
	case err := <-errChan:
		if err != nil {
		...
		for len(clustered) != 0 {
			...
			for service := range clustered {
				systems, err := sh.Services[service].ClusterMembers()
				if err != nil {
					return err
				}
...
func AddPeers(sh *service.Handler, systems map[string]InitSystem) error {
	...
	cluster, err := sh.Services[types.MicroCloud].ClusterMembers()
	if err != nil {
	...
	for peer, cfg := range joinConfig {
		// On a join, roles might still be shuffling, causing dqlite to occasionally block.
		// This is less likely to happen once we have spare cluster members at 7+,
		// so until that time, strictly add sytems one by one, and check that they were successfully added.
