...
validate_system_microceph() {
    ...
    lxc exec ${name} -- sh -ceu "
      ...
      for disk in ${disks} ; do
        ...
        echo \"\${ceph_disks}\" | grep -q \"/dev/disk/by-id/scsi-0QEMU_QEMU_HARDDISK_lxd_\${disk}\"
        count=\$((count + 1))
      done
...
validate_system_lxd_zfs() {
  ...
  echo "${cfg}" | grep -q "volatile.initial_source: /dev/disk/by-id/scsi-0QEMU_QEMU_HARDDISK_lxd_${local_disk}"
...
validate_system_lxd() {
    ...
    # These look like errors so hide them to avoid confusion.
    {
...
reset_snaps() {
  ...
  (
    ...
    if [ -n "${LXD_DEBUG_BINARY}" ] ; then
      lxc file push "${LXD_DEBUG_BINARY}" ${name}/var/snap/lxd/common/lxd.debug
      lxc exec "${name}" -- sh -c "sytemctl reload snap.lxd.daemon"
      lxc exec "${name}" -- sh -c "lxd waitready"
    fi
  )
}
...
restore_system() {
  ...
  echo "==> Restoring ${name} from snapshot snap0 with ${num_disks} fresh disks"
...
setup_lxd_project() {
  ...
  (
    ...
    lxc remote list -f csv | cut -d',' -f1 | grep -q images || lxc remote add images https://images.linuxcontainers.org --protocol simplestreams --auth-type none
...
lxd_vm_waitready() {
  ...
  echo "==> ${name} Awaiting VM agent"
	for round in $(seq 480); do
		if lxc exec "${name}" -- echo "==> ${name} VM agent is ready" 2>/dev/null; then

      return 0
		fi

    # The VM agent seems to get stuck, so restart the process if it takes too long.
    if [ $(($round % 120)) = 0 ]; then
